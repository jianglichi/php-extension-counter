...
#define	_timechk	10                   //server..timeout..
#define	_rowsbegin	"\r\n\nrowsbegin-->"   
#define	_rowsend	"\r\n<--rowsend"             
#define	_fieldbegin	"\r\nfieldbegin-->\r\n"       
#define	_fieldend	"\r\n<--fieldend"       
          ...
          while(size=recv(thisret,buf,_buflen,0)){
               ...
               if(strncmp(buf,_rollback,strlen(_rollback))==0){
                   ...
                   OCITransRollback(poci[ociposi].svchp,poci[ociposi].errhp,OCI_DEFAULT) ;
				     ...
				     OCIErrorGet( poci[ociposi].errhp, ( ub4 ) 1, ( text * ) 0,
				                   &poci[ociposi].errcode, poci[ociposi].errbuf, ( ub4 ) sizeof( poci[ociposi].errbuf ),
				                   OCI_HTYPE_ERROR );
				     printf("msg:## rollback error %d %s \n", poci[ociposi].status, poci[ociposi].errbuf);
               ...
               if((strncmp(tmp,_inputend,strlen(_inputend))==0)){    
                   ...
                   size=realdatalen;
				   size=data_buffer(sqldata,buf,3,0);
                   realdata[size-17]=0;          
				   ...
				   printf("oracle_query \n");
                   fields=oracle_query(&poci[ociposi],realdata,size-17,tmp);
                   ...
                   if(fields>0){
                       ...
                       send_process(_output_begin,strlen(_output_begin),&poci[ociposi]);
                   	   ...
                   	   //get field name
                   	   for (i=0;i<fields;i++){
                   	   		send_process(poci[ociposi].fieldname[i],strlen(poci[ociposi].fieldname[i]),&poci[ociposi]);
							if(i+1<fields){
